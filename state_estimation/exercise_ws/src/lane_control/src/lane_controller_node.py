...
class LaneControllerNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        self.sub_segments = rospy.Subscriber(f"/{veh}/lane_filter_node/seglist_filtered",
                                                 SegmentList,
                                                 self.cbSegList,
                                                 queue_size=1)
    ...
    def car_command(self, timer):
        if not self.points_are_available(Color.YELLOW) and not self.points_are_available(Color.WHITE):
            ...
            if len(self.ypm) !=0 and self.pose_msg.phi > 0: 
                ...
                self.target[1] += 0.15 # moving the follow(target) point to the left.
                #print("PRINTING TARGET: ",target)
            else:
                #print("CONDITION 3 PREVIOUS")
    ...
    def points_y(self):
        ...
        white_var = 0
        with self.lock_for_points:
            yellp = self.seg_points[Color.YELLOW]
            whitep = self.seg_points[Color.WHITE]
            for p in yellp:
                self.yell_y.append(p.y)
            for q in whitep:
                self.white_y.append(q.y)
        #print("Y of YELLOW POINTS = ",self.yell_y)
        if len(self.yell_y) != 0:
            #print("VARIANCE YELLOW Y = ", np.var(self.yell_y))
            yell_var = np.var(self.yell_y)
        #print("WHITE Y POINTS: ", self.white_y)
        if len(self.white_y) != 0:
            #print("VARIANCE WHITE Y: ", np.var(self.white_y))
            white_var = np.var(self.white_y)
        
        return yell_var,white_var
